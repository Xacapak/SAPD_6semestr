 ...
 *           extends - используется для обозначения наследования класса.
 *              Это означает, что новый класс (подкласс) создаётся на основе существующего класса (суперкласса),
 *              унаследовав его свойства и методы.
 *           Comparable - это интерфейс, который позволяет определять порядок между объектами,
 *              выявляя, является ли объект больше, меньше или равным другому.
 *              Интерфейс Comparable содержит один метод — compareTo(T o),
 *              который должен возвращать отрицательное целое число, ноль или положительное целое число,
 *              если текущий объект меньше, равен или больше указанного объекта соответственно.
 *           Интерфейс — это инструмент абстракции, который определяет, что должен делать класс, но не уточняет, как именно.
 *              Интерфейсы позволяют задавать общие контракты для классов, которые могут быть реализованы по-разному.
 *           implements - означает, что класс реализует интерфейс.
 *              Это означает, что класс должен предоставить реализацию для всех методов, объявленных в интерфейсе.
 *              Ключевое слово implements указывает, что интерфейс лишь определяет форму, а класс должен наполнить его кодом.
 *           Iterable - это интерфейс, который позволяет последовательно обходить элементы коллекции.
 *              Он предоставляет механизм для получения итератора — объекта, который абстрагирует доступ к элементам. (hasNext(), next(), remove())
 *           Конструктор  — это специальный блок кода внутри класса, который инициализирует объекты при их создании.
 *           Его задача — настроить внутреннее состояние объекта или присвоить значения его атрибутам.
 *
 */
